...
export default function HeroSection({ stats, currentMCQ, sampleMCQs }: Props) {
    return (
        <section className="bg-gray-50 px-4 pt-6 pb-16 sm:px-6 md:pt-24 lg:px-8">
            <div className="mx-auto max-w-7xl">
                <div className="grid items-center gap-12 lg:grid-cols-2">
                    <div>
                        ...
                        <h1 className="mb-6 text-3xl leading-tight font-bold text-black md:text-5xl lg:text-6xl">
                            Master MCQs with
                            <span className="block text-gray-600">Intelligent Practice</span>
                        </h1>

                        <p className="mb-8 text-sm leading-relaxed text-gray-600 md:text-xl">
                        ...
                        <div className="mb-8 flex flex-col space-y-4 sm:flex-row sm:space-y-0 sm:space-x-4">
                            <Button
                                className="flex items-center justify-center space-x-2 rounded-lg px-8 py-4 text-lg font-semibold transition-all"
                                variant="default"
                                size="lg"
                            >
                        ...
                        <div className="grid grid-cols-2 gap-6">
                            {stats.slice(0, 2).map((stat, index) => (
                                <div key={index} className="rounded-lg border border-gray-200 bg-white p-4 text-center">
                                    ...
                                    <div className="font-bold text-black md:text-xl">{stat.number}</div>
                                    <div className="text-xs text-gray-600 md:text-sm">{stat.label}</div>
                                </div>
                            ))}
                        </div>
                    </div>
